...
def numerical_second_derivative(
    ...
    if scale_h:
        return (f(x0 * (1.0+h)) - 2.0*f(x0) + f(x0 * (1.0-h))) / (x0 * h)**2
    else:                    
        return (f(x0+h)  - 2.0*f(x0) + f(x0-h)) / h**2
...
def newton_raphson(
    ...
    iter = 0
    delta_x = np.finfo(float).max    
    x = seed

    while (
            np.abs(delta_x) > tol_abs 
            and ((x == 0.0) or np.abs(delta_x / x) > tol_rel)
            and iter < max_iters
        ):
...
def qqplot(x, y, quantiles=None, interpolation='linear', ax=None, rug=False,
    ...
    interpolation : {‘inverted_cdf’, ‘averaged_inverted_cdf’, 
                     ‘closest_observation’, ‘interpolated_inverted_cdf’, 
                     ‘hazen’, ‘weibull’, ‘linear’ (default), ‘median_unbiased’,
                     ‘normal_unbiased’}
