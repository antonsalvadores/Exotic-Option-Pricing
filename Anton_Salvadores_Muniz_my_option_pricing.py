...
def sensitivities_european_call_option(
    ...
    Returns:
        sensitivities :math:`\delta, \gamma, \rho, \tetha, \nu` (vega) 
...
def sensitivities_european_option(
    ...
    sensitivities['delta'] = numerical_derivative(price, S_0, h=1.0e-6)
    sensitivities['gamma'] = numerical_second_derivative(price, S_0, h=1.0e-3)
    sensitivities['rho'] = numerical_derivative(
    lambda r: price_european_option(S_0, r, T, sigma, payoff),
    r
)
    sensitivities['theta'] = - numerical_derivative(
    lambda T: price_european_option(S_0, r, T, sigma, payoff),
    T
)
    sensitivities['vega'] = numerical_derivative(
    lambda sigma: price_european_option(S_0, r, T, sigma, payoff),
    sigma
)
...
def implied_volatility(
    ...
    n_iters = 0
    while np.any(abs(delta_sigma) > TOL_ABS) and (n_iters < MAX_ITERS):
    ...
    if n_iters == MAX_ITERS:
        warnings.warm('Maximum number of iterations reached.')
...
def sensitivities_european_option_MC(
    ...
    def derivative_MC(
        ...
        delta_z_0 = z_0 * 1.0e-6 # Es el paso óptimo para la 1 derivada 
        
        derivative_trajectory = (
            (discounted_payoff(z_0 + delta_z_0)
             - discounted_payoff(z_0 - delta_z_0))
            / (2.0 * delta_z_0)
            )
        return (
    ...
    def second_derivative_MC(
        ...
        delta_z_0 = z_0 * 1.0e-3 # Es el paso óptimo para la 2 derivada 
        
        second_derivative_trajectory = (
            (discounted_payoff(z_0 + delta_z_0)
             - 2.0 * discounted_payoff(z_0) 
             + discounted_payoff(z_0 - delta_z_0))
            / delta_z_0**2
            )
        return (
...
def price_european_2_underlyings(
    ...
    # Caso correlación = 1 ó = -1, se calcula con normal univariante
    
    if (abs(np.sqrt(1.0 - rho_square))) < 1.0e-10:
        def integrand_european_option(x):
            return (norm.pdf(x) * payoff(S(S_0_1, r, sigma_1, T, x),
                                        S(S_0_2, r, sigma_2, T, rho*x))
                    )
        return (
            np.exp( - r * T) # Discount factor
            * quad(integrand_european_option, x_minus_infinity, x_plus_infinity)[0]
    ...
    def inner_integrand(x_1, x_2):
        return (
            np.exp(
            - (x_1**2.0 - 2.0 * rho * x_1 * x_2 + x_2**2.0)
                / (2 * (1.0 - rho_square))
              )
            / (2.0 * np.pi * np.sqrt(1.0 - rho_square))
            * (payoff(S(S_0_1, r, sigma_1, T, x_1),
                      S(S_0_2, r, sigma_2, T, x_2))
               )
            )
    ...
    def outer_integrand(x_1):  
        return (
            quad(
                lambda x_2: inner_integrand(x_2, x_1),
                x_minus_infinity, x_plus_infinity)[0]
    )
    return (np.exp( -r * T) * 
            quad(outer_integrand, x_minus_infinity, x_plus_infinity)[0]
            )
...
"""
    ...
    payoff_MC = payoff(
        S(S_0_1, r, sigma_1, T, X_1),
        S(S_0_2, r, sigma_2, T, rho * X_1 + np.sqrt(1 - rho ** 2) * X_2)
    )
...
def price_exotic_option_MC(
    ...
    price_MC = discount_factor * np.mean(payoff_MC)
    stdev_MC = (
    discount_factor * np.std(payoff_MC)
    / np.sqrt(n_simulations))
...
def pricing_control_variate_MC(
    ...
    payoff_MC = payoff(S)
    payoff_CV = payoff_control(S)
    
    price_MC = discount_factor * np.mean(payoff_MC)
    price_MC_CV = discount_factor * np.mean(payoff_CV)
    ...
    beta = (covariance_matrix[0, 1] /
            covariance_matrix[1, 1])
    
    price_control_exact = price_control(S_0, r, T, sigma)
    
    price_MC_corrected = (price_MC - 
                          beta * (
                              price_MC_CV - 
                              price_control_exact)
        )
